...
// parsePortsList turns the big comma-separated string into a deduplicated []int
func parsePortsList(data string) ([]int, error) {
	...
	for _, segment := range strings.Split(data, ",") {
		...
		// ignore "u:" (we are TCP-only for now)
		if after, ok := strings.CutPrefix(segment, "u:"); ok {
			segment = after
		}
		...
		if strings.Contains(segment, "-") {
			...
			if err != nil || p2 < p1 || p2 > 65535 {
				return nil, fmt.Errorf("invalid range: %d-%d", p1, p2)
			}
			for i := p1; i <= p2; i++ {
...
func init() {
	...
	flag.StringVar(&target, "domain", "", "target domain / IP to scan")
	flag.StringVar(&target, "d", "", "target domain / IP to scan (shorthand)")

	flag.IntVar(&worker, "workers", 500, "number of concurrent scanning workers")
	flag.IntVar(&worker, "w", 500, "number of concurrent scanning workers (shorthand)")

	// Custom usage message — called automatically on -help / -h or invalid usage
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage:\n  %s [flags]\n\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "Required flags:\n")
		fmt.Fprintf(os.Stderr, "  -d string, -domain string\n")
		fmt.Fprintf(os.Stderr, "\ttarget domain or IP to scan (required)\n\n")
		fmt.Fprintf(os.Stderr, "Optional flags:\n")
		fmt.Fprintf(os.Stderr, "  -workers, -w int\n")
		fmt.Fprintf(os.Stderr, "\tnumber of concurrent scanning workers (default 500)\n\n")
	}
}
...
func main() {
	...
	// Required check
	if target == "" {
		fmt.Fprintln(os.Stderr, "Error: target is required (-d or --domain)")
	...
	ports, err := parsePortsList(topPorts)
	if err != nil {
	...
	for i := 0; i < worker; i++ {
		wg.Add(1)
		go scanner(portsChan, resultsChan, target, &wg)
	}
